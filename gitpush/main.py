...
def main():
    ...
    print("Welcome to GitPush CLI (powered by Groq)")
    while True:
        user_input = input("\nEnter a prompt to generate a Git command (or type 'exit' to quit): ")
        if user_input.lower() == "exit":
            break
        git_command = get_git_command_from_model(user_input)
        if git_command:
            print(f"\nGenerated Git command: {git_command}")
            execute_git_command(git_command)
        else:
            print("Failed to generate a valid Git command.")
